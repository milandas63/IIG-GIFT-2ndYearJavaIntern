...
public class ManufacturerMaintenance extends JInternalFrame {
    ...
    private void modifyManufacturer() {
        ...
        try {
            // Create a prepared statement to update data in the database
            ...
            // Execute the update query to modify the manufacturer
    ...
    private void deleteManufacturer() {
        // You can show a dialog for selecting a manufacturer to delete
        int selectedRowIndex = table.getSelectedRow();
        if (selectedRowIndex == -1) {
        ...
        int option = JOptionPane.showConfirmDialog(this, "Are you sure you want to delete this manufacturer?", "Confirm Deletion", JOptionPane.YES_NO_OPTION);
        
        if (option == JOptionPane.YES_OPTION) {
            int manufacturerId = (int) table.getValueAt(selectedRowIndex, 0);

            try {
                // Create a prepared statement to delete the manufacturer from the database
                String deleteQuery = "DELETE FROM manufacturer WHERE mfg_id = ?";
                PreparedStatement preparedStatement = conn.prepareStatement(deleteQuery);
                preparedStatement.setInt(1, manufacturerId);

                // Execute the delete query to remove the manufacturer
                int rowsDeleted = preparedStatement.executeUpdate();

                if (rowsDeleted > 0) {
                    JOptionPane.showMessageDialog(this, "Manufacturer deleted successfully.");
                    // Refresh the table to remove the deleted manufacturer
                    loadDataFromDatabase();
                } else {
    ...
    private void searchManufacturer() {
        // You can show a dialog for entering search criteria (e.g., name or abbreviation)
        // Execute a database query with the search criteria and update the table with the results
    }
}
